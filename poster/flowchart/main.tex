...
\newcommand{\res}{%
    \textbf{Die Transversalgschwindigkeit:} \\ \bigskip
    \dots des i-ten Dominos: $v_i = \frac{d+w}{t_i}$ \\ \medskip
    \dots die asymptotische: $v_{as} = \frac{d+w}{t_{as}}$
}
\newcommand{\fig}{%
    \includegraphics[width=\textwidth]{dominoes.png}
...
\begin{tikzpicture}[%
    ...
    \node (res) [result, fit=(table-15-1)(table-18-2)] {\res};
    \graph [use existing nodes, edges={thick, black!50}] {%
        start -> dyn -> ti -> [vhv path=-2.4742cm] res;
